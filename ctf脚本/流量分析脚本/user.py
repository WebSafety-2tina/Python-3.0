...
# 用户输入 IP 和协议来进行筛选
def filter_by_ip_and_protocol(df, ip_input, protocol_input):
    filtered_data = df[(df["源 IP"] == ip_input) & (df["协议"] == int(protocol_input))]
    return filtered_data

# 通过关键词筛选数据
def filter_by_keyword(df, keyword_input):
    filtered_data = df[df["协议"].str.contains(keyword_input, case=False)]
    return filtered_data

# 显示特定协议的操作
def show_protocol_activity(packets, protocol_input):
    protocol_activity = []
    for packet in packets:
        if IP in packet and packet[IP].proto == int(protocol_input):
            if packet.haslayer(Raw):
                data = packet[Raw].load.decode("utf-8", "ignore")
                protocol_activity.append(data)
...
def main(pcap_file):
    ...
    # 用户输入 IP 和协议来进行筛选
    ip_input = input("请输入要筛选的源 IP：")
    protocol_input = input("请输入要筛选的协议：")

    filtered_data = filter_by_ip_and_protocol(df, ip_input, protocol_input)
    logger.info("\n筛选结果：\n")
    logger.info(tabulate(filtered_data, headers=["源 IP", "目的 IP", "协议", "大小"], tablefmt="grid", floatfmt=".2f"))

    # 用户输入要查看的协议的关键词
    keyword_input = input("请输入要筛选的协议关键词：")
    filtered_by_keyword = filter_by_keyword(df, keyword_input)
    logger.info("\n通过关键词筛选结果：\n")
    logger.info(tabulate(filtered_by_keyword, headers=["源 IP", "目的 IP", "协议", "大小"], tablefmt="grid", floatfmt=".2f"))

    # 显示特定协议的操作
    protocol_activity = show_protocol_activity(packets, protocol_input)
    logger.info(f"\n协议 '{protocol_input}' 的操作：\n")
    for activity in protocol_activity:
